...
class GetAllGraFillnAnswers(object):
    ...
    def get_all_gra_fill_answer(self, groupID, taskID):
        url = "{}/sysGrammar/{}/graFill".format(self.baseUrl, groupID)
            ...
            if q.get("currStatus") == 0:
                for r in q.get("steps"):
                    ...
                    if r.get("currStatus") == 0 and len(r.get("subQuestGuide")) != 0:
                        ...
                        answer = {"elapsedSec": 68, "groupID": groupID, "sysID": q.get('id'), "userAnswer": ""}
                        user_answers = []
    ...
    def get_gra_fill_words(self, groupID, taskID, practiceType):
        url = "{}/sysGrammar/{}/graFill".format(self.baseUrl, groupID)
        querystring = {"taskID": "{}".format(taskID)}
        response = requests.request("GET", url, headers=self.headers, params=querystring)
        answer = response.text
    ...
    def get_gra_fill_word_done_data(self, groupID, taskID, practiceType):
        ...
        for q in result:
            ...
            if q.get("currStatus") == 0:
                for r in q.get("steps"):
                    # print("r", r.get("stepName"), r.get("currStatus"))
                    if r.get("currStatus") == 0 and len(r.get("subQuestGuide")) == 0:
                        answer = {"elapsedSec": 0, "groupID": groupID, "practiceType": practiceType, "sysID": q.get('id')}
                        data.append(answer)
        return data